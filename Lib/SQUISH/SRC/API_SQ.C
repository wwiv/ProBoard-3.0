...
SQMSG * MSGAPI SquishOpenArea(byte *name, word mode, word type)
{
  ...
  if ((sq=palloc(sizeof(SQMSG)))==NULL)
    return NULL;

  memset(sq,'\0',sizeof(SQMSG));
...
dword EXPENTRY SquishGetHighWater(SQMSG *sq)
{
  return (SquishUidToMsgn(sq, sq->high_water, UID_PREV));
}


sword EXPENTRY SquishSetHighWater(SQMSG *sq, dword hwm)
{
...
static SQHDR * MSGAPI _SquishGotoMsg(SQMSG *sq, dword msgnum,
...
static sword MSGAPI _SquishUpdateHeaderNext(SQMSG *sq, dword ofs,
...
static sword MSGAPI _SquishReadSq(SQMSG *sq, struct _sqbase *sqb)
{
  lseek(Sqd->sfd, 0L, SEEK_SET);
...
static sword near AddIndex(SQMSG *sq, SQIDX *ix, dword num)
{
  dword where;
  dword old;
...
void EXPENTRY SquishSetMaxMsg(SQMSG *sq, dword max_msgs, dword skip_msgs, dword age)
{
