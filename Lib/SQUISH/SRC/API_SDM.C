...
SQMSG * MSGAPI SdmOpenArea(byte *name, word mode, word type)
{
  SQMSG *mh;

  NW(_junksqd); /* to shut up wcc */

  if ((mh=palloc(sizeof(SQMSG)))==NULL)
  {
    msgapierr=MERR_NOMEM;
    goto ErrOpen;
  }

  memset(mh,'\0',sizeof(SQMSG));
...
static sword EXPENTRY SdmCloseArea(SQMSG *mh)
{
...
static MSGH * EXPENTRY SdmOpenMsg(SQMSG *mh, word mode, dword msgnum)
{
...
static sword EXPENTRY SdmKillMsg(SQMSG *mh,dword msgnum)
{
  dword hwm;
  byte temp[PATHLEN];
  word mn;
...
static sword EXPENTRY SdmLock(SQMSG *mh)
{
  if (InvalidMh(mh))
    return -1;
...
static sword EXPENTRY SdmUnlock(SQMSG *mh)
{
  if (InvalidMh(mh))
    return -1;
...
static UMSGID EXPENTRY SdmMsgnToUid(SQMSG *mh, dword msgnum)
{
  if (InvalidMh(mh))
    return (UMSGID)-1;
...
static dword EXPENTRY SdmUidToMsgn(SQMSG *mh, UMSGID umsgid, word type)
{
  word wmsgid;
  word mn;
...
static dword EXPENTRY SdmGetHighWater(SQMSG *mh)
{
  MSGH *msgh;
  XMSG msg;
...
static sword EXPENTRY SdmSetHighWater(SQMSG *mh,dword hwm)
{
  if (InvalidMh(mh))
    return -1;
...
static sword near _SdmRescanArea(SQMSG *mh)
{
  FFIND *ff;
  byte temp[PATHLEN];
  word mn, thismsg;
